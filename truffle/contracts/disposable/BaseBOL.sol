...
contract BaseBOL is Owned, AddressChecker {
    ...
    // We fetch basic move info from the Meta BOL
    struct MoveInfo {
    ...
    function BaseBOL(address _metaBOL) {
    ...
    function setMetaBOL(address _metaBOL)
        onlyContractOwner
        notNull(_metaBOL)
    returns(bool) {
        ServiceChanged("MetaBOL", metaBOL, _metaBOL);
        metaBOL = MetaBOLInterface(_metaBOL);
        return true;
    }
    ...
    /**
     * Reserve pre-deployed BOL for some parties (prepare for deal).
     */
    function reserve(
        address _shipment,
        uint256 _price,
    ...
    returns(bool) {
        ...
        uint intermediariesCount = metaBOL.getIntermediariesCount();
        ...
        for (uint8 i = 0; i < intermediariesCount; i++) {
            party = metaBOL.intermediaries(i);
        ...
        require(_reserveCallback(_shipment, _price, _seller, _buyer, _escrow, _intermediaries, _payments));
    ...
    function _approveOwnershipTransfer(uint8 _move) internal {
        _payFee(_move);
        ShipmentInterface Shipment = ShipmentInterface(shipment);
        assert(Shipment.approveOwnershipTransfer(buyer));
    ...
    function _rejectOwnershipTransfer(uint8 _move) internal {
        //_releaseFee(_move);
        ShipmentInterface Shipment = ShipmentInterface(shipment);
        assert(Shipment.rejectOwnershipTransfer());
    ...
    function _controller() internal constant returns(ControllerInterface) {
        return ControllerInterface(metaBOL.controller());
    }
