...
/**
 * Meta bol is an inheritable contract that implements a storage of some
 * typical workflow, that we don't want to duplicate for each bol contract
 * of particular type, say, for some jurisdiction.
 *
 * Assume we need to implement a bol for some jurisdiction.
 * Then we create a contract called MetaBOLOfSomeCountry.sol,
 * and it inherits all of the stuff from MetaBOL.
 * Then, in its constructor, we define all of the `Moves`, `roles` and so on.
 *
 * See MetaBOLUkraine. for details.
 *
 * We do so to minimize deployment gas costs, since storage space is expensive.
 */
contract MetaBOL is Owned {
    ...
    function MetaBOL(address _controller) {
