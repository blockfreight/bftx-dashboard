...
//   event PropyComission(address indexed bol, uint256 companyComission, uint256 networkGrowthComission);
...
//   // ---------------------------------------------------------------------------
//   // Constuctor
//   // Initialize BOL metadata
//   function BOL(address _shipmentId, address _brokerId, address _buyerId, address _buyer_wallet,
...
//       address _bolCreatorAddress) {
//     propyContractAddress=msg.sender;
//     bolCreatorAddress=_bolCreatorAddress;
...
//     Shipment prop = Shipment(shipmentId);
...
//   function documentSaveTitleBOLLocal(string _date, string _localHash) {
...
//     //updates the owner of the shipment
//     Shipment prop = Shipment(shipmentId);
//     prop.approveOwnershipTransfer(buyerId);
//     status=Status.titleBOLAppoved;
//     StatusUpdate(status);
...
//       ShipmentManager proeprtyManager = ShipmentManager(propyContractAddress);
//       assert(proeprtyManager.setShipmentToPendingState(shipmentId));
...
//    if (msg.sender == contractOwner || msg.sender == bolCreatorAddress){
//       suicide(msg.sender);
//     }
//   }
